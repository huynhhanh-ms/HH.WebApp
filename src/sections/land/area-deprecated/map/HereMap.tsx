    ...
    () => {
      ...
      if (!map.current) {
        ...
        if (mapRef.current) {
          ...
          let mapEvents = new H.mapevents.MapEvents(newMap);
          ...
          // Add panning and zooming behavior to the map
          mapEvents = new H.mapevents.MapEvents(newMap);
...
function calculatePolygonCenter(polygon: any) {
  let latSum = 0;
  let lngSum = 0;
...
function createTextElement(text: string, position: H.geo.IPoint | H.geo.MultiPoint) {
...
function createPolygon(area: PolygonArea) {
  const linePoints = area?.coords?.reduce<number[]>((prev, cur) => [...prev, cur.lat, cur.lng, 100], []);
